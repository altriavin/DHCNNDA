...
parser.add_argument('--dataset', default='lncRNA-drug', help='dataset name: diginetica/Nowplaying/sample')
...
def main():
    train_data = pickle.load(open("datasets/" + opt.dataset + "/train_pos_30000.txt", "rb"))
    test_data_pos = pickle.load(open("datasets/" + opt.dataset + "/test_pos_5000.txt", "rb"))
    test_data_neg = pickle.load(open("datasets/" + opt.dataset + "/test_neg_5000.txt", "rb"))

    if opt.dataset == "ncRNA-disease":
        n_node = 556
    elif opt.dataset == "lncRNA-drug":
        n_node = 1816
