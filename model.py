...
def test(model, test_data):
    ...
    for i in slices:
        ...
        for idd in range(model.batch_size):
            index, score = find_k_largest(K, scores[idd])
            all_index.append(index)
            all_scores.append(score)
        all_scores = torch.softmax(torch.FloatTensor(all_scores), dim=1)
        all_index = np.array(all_index)
        all_scores = np.array(all_scores)
        tar = trans_to_cpu(tar).detach().numpy()
        for prediction, prediction_scores, target in zip(all_index[:, :K], all_scores[:, :K], tar):
            if len(np.where(prediction == target)[0]) == 0 or np.where(prediction == target)[0][0] >= K:
                prediction_index_probability = 0
            else:
                prediction_index = np.where(prediction == target)[0][0]
                prediction_index_probability = prediction_scores[prediction_index]
            auc_list.append(prediction_index_probability)
    return auc_list
...
def train_test(model, train_data, test_data_pos, test_data_neg):
    ...
    test1 = test(model, test_data_pos)
    test2 = test(model, test_data_neg)
    # print(f'test1: {test1}\n'
    #       f'test2; {test2}')
    y_score = test1 + test2
    y_true = [1] * len(test1) + [0] * len(test2)
